...
NSString * HomeworkAssignmentTwo$ModuleRequireFormat = @"(function(exports){"
...
@implementation HomeworkAssignmentTwoObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_homeworkassignmenttwo);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create HomeworkAssignmentTwo global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "HomeworkAssignmentTwo" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_homeworkassignmenttwo = [[HomeworkAssignmentTwoObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_homeworkassignmenttwo];
	
	NSString *_homeworkassignmenttwoNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _homeworkassignmenttwoNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_homeworkassignmenttwo valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:HomeworkAssignmentTwo$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.homeworkassignmenttwo.kroll" 
